...
describe("public documentation", () => {
  ...
  it("separates universal skill installation from MCP server setup", async () => {
    ...
    expect(readme).toContain("https://github.com/ZaK3939/GamePlayerLens/tree/v0.7.0/skills/game-player-lens");
    expect(readme).toContain("npx skills list");
    expect(readme).toMatch(/installs[\s\S]*skill[\s\S]*does not install[\s\S]*MCP server/i);
    expect(readme).toContain("npx --yes --package=github:ZaK3939/GamePlayerLens#v0.7.0 game-player-lens doctor");
    expect(readme).toContain('"command": "npx"');
    expect(readme).toContain('"--package=github:ZaK3939/GamePlayerLens#v0.7.0"');
