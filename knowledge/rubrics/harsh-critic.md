...
- price選択時に Field | Value の`Pricing Decision Trace`がなく、次のfieldをこの順の各行で分けない場合は差し戻す: primary objective; other objectives; base price; package / edition; region; launch discount; post-offer price; value / quality signal; matched competitor evidence; matched player-response evidence; official rules checked at; success signal; observation window; guardrail; revisit condition。8列1行に詰めたtraceは差し戻す。
- primary objectiveが`net-revenue` / `paid-reach` / `qualified-feedback` / `positioning`以外、2つ以上、またはother objectivesを第二のsuccess metricにした場合は差し戻す。手段と目的が一致しない案も差し戻す。安いほど販売が増える、高いほど品質が高い、を事実とした場合は差し戻す。value / quality signalを仮説ではなく事実とした場合も差し戻す。
- Steam向けのbase priceまたはlaunch discount案を勧める場合、公式Steamworks URL + accessedAtがなく、現行pricing / discount ruleのminimum threshold、率、期間、cooldownの実行可能性を確認していない場合は差し戻す。Steam launch discountが現行上限の10–40%・7–14日でない、release後に設定する、地域minimumを下回る場合は差し戻す。単一developer事例またはcommunity commentだけを市場効果の証明にした場合も差し戻す。platform feasibilityは`rule-valid` / `rule-invalid` / `not-steam` / `unresolved`以外で書かない。
- `store-reveal`、`release-date`、`launch`を判断するのに`### Steam Release Readiness`、Status Selected、7行の順序付きgate表がなければ差し戻す。headerは Gate | Current status | Evidence status / ID | Official source | Date / earliest completion | Owner | Next action。行は Onboarding / app credit、App configuration、Store Presence、Game Build、Coming Soon、Pricing / launch offer、Manual release の順。App configuration欠落は差し戻す。
- Blocking gateが上記7名または`none`以外の場合、Steamworks未接続時のdeveloper申告をobservedへ昇格した場合、official source / accessedAt、earliest release date、blocking gateを欠く場合は差し戻す。Steam Release Readinessをplayer fun / 需要 / 売上の証明にした場合、`Release App`自動化を勧めた場合、Steamworks credential / bank / tax / identityを要求した場合も差し戻す。current gateが`store-reveal` / `release-date` / `launch`以外なら`### Steam Release Readiness`見出しと明示的なN/A理由を要求し、表の欠落は不合格にしない。
