...
6. 競合 appid 群を `derive_personas` に渡します。通常の3〜5 personaでは `reviewsPerPolarity=8`、レビュー根拠を広く監査するときだけ最大25を使います。返り値の派生素材パック全体を、`save_artifact` のkind=`intel`、sourceTool=`derive_personas`、返り値のmeta.observedAtで保存し、返されたrepository-relative pathをEvidence Indexへ記録します。1 MiB制限で保存できない場合は`reviewsPerPolarity`を下げて再実行し、根拠を黙って省略しません。保存済みの schema とレビュー出典から指定件数の異なる persona JSON を生成し、その後に各 JSON を `save_persona` で保存します。`derive_personas` → 派生素材パックの保存 → `save_persona` の順序を逆にしてはいけません。
