...
2. 対象を `steam_search` と `steam_fetch` で解決します。`steam_fetch.localizedStorefronts` はenglish / japanese / germanのrequested localeであり、Steamのfallback copyである可能性を残します。Steam Sonarのゲーム別dashboardへのdeep linkは `referenceLinks.steamSonar`、公式ストアは`steamStore`、現在値と履歴を人間が確認するSteamDBは`steamDb`です。リンク先の未保存データを取得済み根拠として扱いません。
...
6. 競合 appid 群を `derive_personas` に渡します。通常の3〜5 personaでは `reviewsPerPolarity=8`、レビュー根拠を広く監査するときだけ最大25を使います。返された schema とレビュー出典から指定件数の異なる persona JSON を生成し、その後に各 JSON を `save_persona` で保存します。`derive_personas` → `save_persona` の順序を逆にしてはいけません。
