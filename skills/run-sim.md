...
- `conceptTest`は第三者のconcept理解testを表す検証済みJSONです。`testedAt`、`stimulusId`、提示内容と手順、`recruitment`、target player定義、`questionsAsked`、1〜50件のparticipant観測を渡せます。`participantId`は匿名の仮名IDだけを使い、氏名、email、連絡先、自由記述内の個人情報を渡しません。
- `conceptTestDiagnostics`はこのbounded sampleの記述件数です。`understoodAction`、`understoodReward`、`interest`を別々に読み、母集団比率、購入予測、固定合格thresholdへ変換しません。
...
3. `Concept Test Trace`: 提示したstimulus、募集条件、質問、行動理解、報酬理解、興味、混乱点、deviationを追跡する。未入力ならmissingとする。
...
4b. `conceptTest`がある場合は、正規化済みpayloadを変更・要約せず、直ちに`save_artifact`のkind=`intel`、sourceTool=`manual`で保存します。検証済みの`testedAt`だけをtop-level `observedAt`へ使い、返されたrepository-relative pathをEvidence Indexとrun evidenceへ入れます。`oneSentencePromise`と`unaidedSummary`、`understoodAction`、`understoodReward`、`interest`を別々に照合し、confusionとprotocol deviationを残します。participant countをconversion、母集団比率、需要へ変換しないこと。`interest`は試したいという回答であり、purchaseや継続を証明しません。
...
- conceptTest入力時は正規化済みpayloadのmanual intel path、Concept Test Trace、行動理解・報酬理解・興味の別集計、匿名性、bounded sampleの解釈限界を記録した。
