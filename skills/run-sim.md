...
4d. `playtestSession`がある場合は、直ちに`playtestSessionEvidence.resultHandle`を使って`save_artifact`をkind=`intel`、target、id、resultHandleだけで呼びます。返されたpathをEvidence Indexとrun evidenceへ入れ、payloadを転記・要約しません。Action、system response、friction、`rewardSignal`、human participantの`humanReport`を別々に読み、protocol mismatch、deviation、prior knowledgeをlimitationへ残します。AI-operated sessionに人間の感情報告を補完しません。one bounded sessionの成功・失敗・感想をcompletion rate、retention、需要へ変換しません。
...
- playtestSession入力時は`playtestSessionEvidence.resultHandle`で正規化済みpayloadをexact-saveしたmanual intel path、build / task / controlsの照合、時系列Action → response → rewardSignal、humanReportの有無、deviation、one-sessionの解釈限界を記録した。
