...
3. `Concept Test Trace`: brief revision、実際に提示したpromise、stimulus、募集条件、質問、行動理解、報酬理解、興味、混乱点、deviationを追跡する。未入力ならmissingとする。
4. `Promise-Delivery Trace`: 購入前のAppeal Promiseをassetへ、購入後のDelivered Experienceをbuild momentとplaytest / telemetryへ対応付ける。
5. `Funnel Health`: impression → store visit → wishlist → demo start → demo completion → purchase → retained playを、source、cohort、window、欠損付きで分離する。
6. `Milestone Readiness`: current gateのplayer evidenceとasset evidenceを判定し、日付だけでpassにしない。
7. `Experiment Queue`と`Survival Scenarios`: 最大3件の検証と、project固有のcost / fee / refund / tax / runway assumptionを記録する。
