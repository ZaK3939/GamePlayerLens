...
- `conceptTest`は第三者のconcept理解testを表す検証済みJSONです。`testedAt`、`stimulusId`、再検証なら対になる`parentStimulusId`と`changeSummary`、比較設計の`changedVariables`と`invariantsKept`、任意の`projectBriefRevision`、実際に提示した`promiseShown`、提示内容と手順、`recruitment`、target player定義、`questionsAsked`、1〜50件のparticipant観測を渡せます。`participantId`は匿名の仮名IDだけを使い、氏名、email、連絡先、自由記述内の個人情報を渡しません。schemaが自動検出する個人情報はemail形式だけなので、それ以外は送信前に除去します。
...
- `firstContactTest`は実際のasset表示contextを使った第三者testの検証済みJSONです。`assetId`、任意の親assetと比較設計、asset type / description、device・viewport・duration・sound・order、募集条件、質問、1〜50件の`understoodTheme` / `understoodAction` / `understoodReward` / `immediateReject`を渡せます。固定枚数や固定秒数を合格条件にしません。
